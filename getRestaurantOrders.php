...
$sql = "SELECT mo.OrderId, ca.Name as CustomerName, ca.Contact as CustomerContact, ca.LatLong, concat(ca.Address,',',ca.City,',',ca.Pincode,',',ca.State) as CustomerAddress, mo.TotalPrice, mo.DeliveryCharge, mo.GrandTotal, mo.PaymentMode, mo.Instruction, mo.Status, date_format(mo.OrderDatetime,'%d-%b-%Y %H:%i') as OrderDatetime, date_format(mo.PreparingDatetime,'%d-%b-%Y %H:%i') as PreparingDatetime, date_format(mo.ReadyDatetime,'%d-%b-%Y %H:%i') as ReadyDatetime, date_format(mo.PickedUpDatetime,'%d-%b-%Y %H:%i') as PickedUpDatetime, date_format(mo.DeliveredDatetime,'%d-%b-%Y %H:%i') as DeliveredDatetime, date_format(mo.CancelledDatetime,'%d-%b-%Y %H:%i') as CancelledDatetime, mo.CancellationMsg, mo.RiderId, mo.SelfAccept FROM MyOrders mo join CustomerAddress ca on mo.CustAddId = ca.CustAddId where mo.Status != 7 and mo.RestId = $restId ORDER by mo.OrderDatetime DESC";
